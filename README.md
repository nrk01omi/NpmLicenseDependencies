...
3. **結果** — テーブルで閲覧。「すべて / NG のみ / OK のみ / 不一致のみ」フィルタと名前・ライセンスの絞り込み、
   ...
   （複数の親から要求されるものは 2 回目以降「既出」として薄く表示。CSV も既定で同じ行を出力し、「既出」列で絞り込めます）。画面先頭の「0. 動作仕様・使い方」から仕様ページを別ウィンドウで開けます。
...
| `--order <o>` | CSV の並び順。`tree`（既定。親子順＝直接依存の直下に子・孫を名前順で深さ優先）または `depth`（深さ順、同じ深さは名前順） |
...
| 要求元 | このライブラリを要求しているライブラリすべて（`name@version`、`; ` 区切り）。再帰調査のときのみ |
| 既出 | 画面と同じ親子順展開で、複数の親から要求される行の 2 回目以降なら `既出`、実体の行は空。`--no-duplicates` や深さ順では常に空 |
| 表示階層 | 画面のインデント段数（親子順のときの表示上の深さ）。同じライブラリでも親が違えば値が変わります |
| 表示上の親 | その行が画面でぶら下がっている親（`name@version`）。根（直接依存）は空 |
...
親子順のとき、複数の親から要求されるライブラリは画面では 2 回目以降も親の下に「既出」として表示されます。
CSV も既定で**画面と同じ行**を出力します（GUI「CSV も画面と同じ行（既出を含む）」／CLI は既定 ON、`--no-duplicates` で OFF）。

- 「既出」「表示階層」「表示上の親」列で、Excel のフィルタで既出行を外したり、特定の親の塊だけを見たりできます
- 「既出」行はライブラリの情報（ライセンス・依存・脆弱性など）は実体の行と同じ内容です。集計するときは `既出` が空の行だけを対象にしてください
- このツールで CSV を読み込むときは「既出」行は自動的に読み飛ばし、表側で親子順に展開し直します
- OFF にすると従来どおり 1 行 1 パッケージ（`name@version`）になります
